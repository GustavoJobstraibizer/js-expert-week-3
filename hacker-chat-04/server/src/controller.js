...
export default class Controller {
  ...
  async joinRoom(socketId, data) {
    ...
    // avisa a rede que o novo usuario conectou-se
    this.broadcast({
      socketId,
      roomId,
  ...
  broadcast({ socket, roomId, event, message, includeCurrentSocket = false }) {
    const usersOnRoom = this.#rooms.get(roomId);

    for (const [key, user] of usersOnRoom) {
      if (!includeCurrentSocket && key === socket) continue;
